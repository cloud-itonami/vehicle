(ns vehicle.app
  "etzhayyim-project-vehicle appview — reagent + re-frame, view built from
  jp-go-dds (デジタル庁デザインシステム) hiccup.

  This is a faithful port of the previous SvelteKit scaffold
  (`svelte/src/routes/+page.svelte`, provenance:
  `60-apps/etzhayyim-project-vehicle/svelte/src/routes/+page.svelte`): a
  static info page describing this appview surface itself — its
  ...
  dispatcher or the MCP router — it was pure static markup describing the
  surface's own metadata, and this port adds no vehicle-manufacturing-graph
  ...
  map keys), same pattern as the landed reference this was copied from
  (`cloud-itonami/app-tia`'s `tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration: `../src/app.ts` (the
  thin edge dispatcher Worker) is untouched. The one other TypeScript file
...
(def default-db
  {:title "Ai etzhayyim Project Vehicle"
   :project "etzhayyim-project-vehicle"
   :name "etzhayyim-project-vehicle"
   :kind "cloudflare surface"
   :route-count 1
   :routes ["v3h1cl01.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DEPLOY_AT"
   ...
   :xrpc? true
   :relative-path "cljs/src/vehicle/app.cljs"})
